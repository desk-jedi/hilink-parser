...
def create_table(data_sheet) -> None:
    ...
    headers = [
        "IP адрес", "Модель", "Серийный номер", 
        "IMEI", "ICCID/SIM", "Моб. номер"
    ]
...
def main() -> None:
    ...
    for url in urls:
        ...
        try:
            if count_address != 1:  # Пропускаем первый адрес.
            ...
            print(f"Не удалось получить данные по адресу {url_format}. Проверьте корректность URL и доступность сайта")
            continue  # Переходим к следующему адресу.

        block = BeautifulSoup(html, "lxml")  # Парсим HTML-код.

        info_values = block.find_all("td", class_="info_value")  # Извлекаем нужные данные.

        output_name = "output-" + str(datetime.datetime.now())[:10] + ".log"  # Название лог-файла.
        ...
        try:
            ...
            print(f"Данные по адресу {url_format} успешно получены")
        except Exception:
            print(f"Не удалось получить данные по адресу {url_format}. Проверьте корректность URL и доступность сайта")
            continue  # Переходим к следующему адресу.
